...
public class UserLogServiceImpl implements UserLogService {
	...
	public int signIn(String username) {
		...
		if (DateHelper.isYesterday(date)) {
			num++;
			if (num%7==0) {
				userAccount.setAvailablePrincipal(userAccount.getAvailablePrincipal()+500);
				userAccount.setHistoryPrincipal(userAccount.getHistoryPrincipal()+500);
			}else {
				userAccount.setAvailablePrincipal(userAccount.getAvailablePrincipal()+100);
				userAccount.setHistoryPrincipal(userAccount.getHistoryPrincipal()+100);
			}
		}else {
			num = 1;
			userAccount.setAvailablePrincipal(userAccount.getAvailablePrincipal()+100);
